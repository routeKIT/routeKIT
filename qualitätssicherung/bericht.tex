...
Ein "break" entfernen, das nach der ersten Ein-Kanten-Straße das Erzeugen von Kanten abbricht.
}{
behoben
}
...
Die Projektion findet im Geokoordinatensystem statt und nicht nach der Mercator-Projektion (im SMT-Koordinatensystem)
}{
...
Wenn im ProfileManager-Dialog Vorberechnungen gelöscht werden und er mit "Enter" geschlossen wird, erscheint keine Warnung. Beim Klick auf den "OK"-Knopf erscheint Sie.
}{
Der Code, der die Warnung erzeugt, liegt im Programmcode des Knopfes und nicht in der Methode des ProfileManagers, die den Dialog schließt.
}{
Den Code, der die Warnung erzeugt, in den ProfileManager verschieben.
}{
behoben
}
\end{itemize}
...
Das Rendern der Kacheln ist auf niedrigen Zoomstufen zu langsam.
}{
...
\bug{routeCalcAllEdges}{Routenberechnung über alle Kanten}{
Die Routenberechnung ist zu langsam, auch für kurze Routen.
}{
Der Algorithmus fügt zu Beginn der Berechnung alle vorhandenen Kanten in einen Heap ein, obwohl die eigentliche Routenberechnung durch ArcFlags oder nah beieinander liegende Start- und Zielpunkte möglicherweise nur auf einen Bruchteil der Kanten zugreifen muss.
}{
Mehrere Datenstrukturen "lazy" initialisieren. (Nicht eingefügte Kanten sind "unendlich" weit weg\ldots)
}{
behoben
}
...
Es werden für jeden der Dijkstras am Anfang alle Kanten des Graphen in den Heap eingefügt, auch wenn später möglicherweise nie darauf zugegriffen wird.
}{
...
Begonnene Routenberechnungen werden dem RouteModel nicht mitgeteilt, sodass die MainView nicht erfahren kann, dass gerade eine Berechnung läuft.
}{
...
\test{restriction}{Restriction}
     {Es wird für verschiedene Beschränkungen (\code{Height}-, \code{Width}-, \code{Weight}-, \code{VehicleType}-, \code{Multiple}-, \code{NoRestriction}) die \code{allows}-Methode für die Profile „PKW (Standard)“ und „LKW (Standard)“ getestet.}
...
\test{graphIndex}{GraphIndex}
     {Es werden beide \code{GraphIndex}-Implementierungen (\code{Array}- und \code{Tree}-) getestet. Dazu werden sie mit demselben Graphen (einem Gitter aus unverbundenen Straßenstücken) initialisiert.
     Dann werden verschieden Bereiche von Kanten angefordert und verglichen.
     Danach werden noch Koordinaten mit dem Index projiziert.}
     {die resultierenden Kantenmengen und der Punkt auf der Kante jeweils mit den erwarteten Werten übereinstimmen.}
     {bestanden}
\test{tileCacheCache}{TileCache}
     {Es wird das Speichern von Kacheln im \code{TileCache} getestet.}
     {eine Kachel innerhalb von 100\,ms nach der ersten Anfrage im Cache liegt.}
     {bestanden}
\test{tileCachePrefetch}{TileCache}
     ...
     {nach dem Anfragen einer Kachel innerhalb von 100\,ms benachbarte Kacheln ebenfalls im Cache liegen.}
     {bestanden}
\test{tileCacheCleanup}{TileCache}
...
\test{routeModel}{RouteModel}
     {Es wird das Beobachtermuster der \code{RouteModel}-Klasse getestet. Dazu wird überprüft, ob in verschiedenen Situationen Ereignisse an alle Listener ausgegeben werden, indem ein Testlistener registriert wird und verschiedene Änderungen an den Daten durchgeführt werden.}
     {die gemessene Anzahl der ausgelösten Ereignisse an allen Stellen mit der erwarteten Anzahl übereinstimmt.}
     {bestanden}
\test{weights}{Weights}
...
\test{fileUtil}{FileUtil}
     ...
     {\code{checkMapName}/\code{checkProfileName} zulässige Karten-/Profilnamen akzeptiert und unzulässige ablehnt.}
     {bestanden}
\end{itemize}
