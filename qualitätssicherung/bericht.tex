...
\bug{profileNoWarning}{Profilverwaltung: Warnung bei Enter}
    ...
    {behoben}
\bug{testCLIHelp}{CLI Help Test}
    {Der Test für die Ausgabe des Hilfetext für das Konsoleninterface schlägt auf Windows-Systemen
    mit einer \code{ArrayIndexOutOfBoundsException} fehl,
    obwohl er auf Linux-Systemen funktioniert.}
...
\bug{deleteProfileWhileAddingToMap}{Karten- und Profilverwaltung Inkonsistenz}
    {In der Kartenverwaltung ist ein Profil für die aktuelle Karte vorberechnet
...
\bug{currentProfileNotExisting}{Gelöschtes aktuelles Profil}
    {Das einzige nicht-Standard-Profil ist ausgewählt. In der Kartenverwaltung wird dieses Profil (im Hinzufügen-Dialog) gelöscht
...
\bug{noRouteWirdBerechnet}{Falscher Infotext wenn keine Route gefunden wird}
	...
	{Zu Beginn der Routenberechnung wir startCalculating() aufgerufen, wodurch "Route wird berechnet..." angezeigt wird.
...
\testfall
    ...
    {Eine Route, die den Einbahnstraßenabschnitt enthällt wird gefunden.}
...
Die Abdeckung des Quellcodes im "src"-Ordner (also ohne Testfälle) während dem Commit \code{3035972f6643f6d5b53002d1823e99f5de07d9fb} ist:\\
\begin{longtable}{||c|c|c|c||}
Testteil & Instruction Coverage & Line coverage & Branch coverage\\ \hline\hline
RobotUI & 78.1\%&79.7\%&57.0\%\\ \hline
Unit Tests & 50.5\%&50.1\%&44.1\%\\ \hline \hline
Merged & 86.2\%&88.0\%&65.7\%\\ \hline
\end{longtable}
...
Die fehlende Abdeckung von \code{TerminalCLI} ist dadurch zu erklären, dass diese Klasse die Benutzung der Import- und Vorberechnungsfunktion in einer Kommandozeile abstrahiert. Dies 

Die auch eher niedrige Abdeckung der anderen Klassen soll hier
...
Diese Problematik wird an eine anderen Stelle wird bei der Benutzung des Java-Konstrukts "try-with-Resouces" noch deutlicher, wie man in Abbildung \ref{fig:tryWithResources} erkennen kann:
...
Es sind die Anweisungen, die dafür sorgen, dass die Resourcen (\code{RandomAccessFile},\code{FileChannel},\code{FileInputStream},\code{DataInputStream}) auch im Fehlerfall ordnungsgemäß geschlossen werden (und auch, dass die anderen Resources geschlossen werden, wenn das schließen einer andere fehlschlägt). So sind allein im sichtbaren Programmcode 96 Befehle, die nicht überdeckt sind.
