...
Hinzufügen von \code{isCalculated()} zum RouteModel; siehe auch Abschnitt \ref{change:routeCalcKeineRoute}.
}{
behoben
}
\end{itemize}
...
\test{historyEntry}{HistoryEntry}{
  ...
  ein HistoryEntry nach Anwenden von fromString auf den von toString erzeugten String den selben Start- und Zielpunkt sowie Zeitstempel enthält wie davor.
}{bestanden}

\test{edgeProps}{EdgeProperties}{
  Es werden die Getter von EdgeProperties und insbesondere getMaxSpeed getestet.
}{
  die Getter die korrekten Werte zurückgeben und getMaxSpeed, je nach Straßentyp und übergebenem Profil, die korrekte Höchstgeschwindigkeit des gegebenen Fahrzeugtyps auf diesem Straßentyp liefert.
}{bestanden}
...
\test{restriction}{Restriction}{
  ...
  bei jeder getesteten Restriction allows für defaultCar und defaultTruck die korrekte Ausgabe liefert.
}{bestanden}

\test{edgeBasedGraph}{EdgeBasedGraph}{
  Es wird die Speicher- und Lade-Funktion von EdgeBasedGraph getestet.
}{
  der EdgeBasedGraph nach dem Speichern und anschließenden Laden die gleichen Edges und Turns enthält wie davor.
}{bestanden}

\test{graph}{Graph}{
  Es werden die Getter sowie die Speicher- und Lade-Funktion von Graph getestet.
}{
  die Getter die korrekten Werte zurückgeben und der Graph nach dem Speichern und anschließenden Laden die gleichen Nodes und Edges enthält wie davor.
}{bestanden}
...
\test{graphIndex}{GraphIndex}{
  ...
  %TODO Umformulieren und an den Rest anpassen.
}{
  die resultierenden Kantenmengen und der Punkt auf der Kante jeweils mit den erwarteten Werten übereinstimmen.
}{bestanden}

\test{tileCacheCache}{TileCache Cache}{
  Es wird das Speichern von Kacheln im TileCache getestet.
}{
  eine Kachel innerhalb von 100ms nach der ersten Anfrage im Cache liegt.
}{bestanden}

\test{tileCachePrefetch}{TileCache Prefetch}{
  Es wird das Prefetching (vorsorgliche Berechnung von benachbarten Kacheln, auch wenn diese noch nicht direkt angefragt wurden) des TileCache getestet.
}{
  nach dem Anfragen einer Kachel innerhalb von 100ms benachbarte Kacheln ebenfalls im Cache liegen.
}{bestanden}

\test{tileCacheCleanup}{TileCache Cleanup}{
  Es wird das Entfernen von Kacheln aus dem TileCache getestet.
}{
  nach dem Anfragen von $50^2$ anderen Kacheln die älteste Kachel aus dem Cache entfernt wird.
}{bestanden}

\test{arcflags}{ArcFlags}{
  Es wird die Speicher- und Lade-Funktion der ArcFlags getestet.
}{
  nach dem Speichern und anschließenden Laden die gleichen ArcFlags gesetzt sind wie davor.
}{bestanden}
...
\test{routeModel}{RouteModel}{
  ...
  die gemessene Anzahl der ausgelösten Events an alles Stellen mit der erwarteten Anzahl übereinstimmt.
}{bestanden}
...
\test{osmParser}{OSMParser}{
  Es wird die Verarbeitung verschiedener Karteneigenschaften
  (verschiedene Abbiegetypen, Einbahnstraßen, Ampeln, Kreisverkehre, \ldots) sowie das Erkennen von Kartenfehlern (Eigenschaften ohne Name, Eigenschaften ohne Wert, fehlende Koordinaten, Ungültige Koordinaten,\ldots) durch den OSMParser getestet.
}{
  alle gültigen Strukturen korrekt in den Graph übernommen werden und ungültige Karten einen Fehler auslösen. %TODO Bessere Formulierung: Fehler auslösen klingt fast so, als ob das Programm abstürzen würde oder so, aber mir fällt nichts besseres ein gerade...
}{bestanden}

\test{osmWay}{OSMWay}{
  Es wird die Verarbeitung verschiedener OSM Tags (Straßentyp, Kreisverkehr, \ldots) durch die OSMWay-Klasse getestet.
}{
...
\test{profileMapComb}{ProfileMapCombination}{
  ...
  die ProfileMapCombination nach dem Speichern und anschließenden Laden das gleiche Profil und die gleiche Streetmap sowie, falls vorhanden, die gleiche abgeschlossene Vorberechnung (Weights, ArcFlags, benötigte Zeit) enthält wie davor.
}{bestanden} %TODO Den entsprechenden Test im Quellcode umbenennen von TestPrecalculationSaveLoad in TestProfileMapCombinationSaveLoad?

\test{turnRestriction}{TurnRestriction}{
  Es wird die Funktionalität von TurnRestrictions getestet.
}{
  eine TurnRestriction ihren Start- und Ziel-OSMWay korrekt zurückgibt und allowsTurn korrekt entscheidet, ob ein Turn auf einen bestimmten OSMWay erlaubt ist oder nicht.
}{bestanden}

\test{profile}{Profile}{
  Es werden equals und clone sowie die Setter, Getter und die Speicher- und Lade-Funktion der Profile getestet.
}{
  equals und clone erwartungsgemäß funktionieren, die Setter die verschiedenen Werte (VehicleType, Height, Width, etc.) korrekt setzen, die Werte von Standardprofilen jedoch nicht verändern, die Getter die korrekten Werte zurückgeben und das Profil nach dem Speichern und anschließenden Laden die gleichen Werte enthält wie davor.
}{bestanden}

\test{fibHeap}{FibonacciHeap}{
  Es werden die verschiedenen Operationen (Einfügen/Entfernen von Elementen, Ändern der Priorität) des FibonacciHeaps getestet.
}{
  die deleteMin-Methode nach dem Hinzufügen/Entfernen von Elementen und Ändern von Prioritäten immer korrekt das Element mit der niedrigsten Priorität zurückgibt.
}{bestanden}

\test{coords}{Coordinates}{
  Es werden distanceTo, angleBetween sowie toString/toSMT und fromString/fromSMT getestet.
}{
  distanceTo die korrekte Entfernung zwischen zwei Punkten zurückgibt, angleBetween korrekt den zwischen zwei (durch zwei Punkte angegebenen) Geraden eingeschlossenen Winkel berechnet und durch die Anwendung von fromString/fromSMT auf den/die von toString/toSMT erzeugten String/SMT-Koordinaten Coordinates mit den gleichen Koordinaten wie die ursprünglichen Coordinates erzeugt werden. %TODO etwas verschachtelt... Umformulieren?
}{bestanden}

\test{pointOnEdge}{PointOnEdge}{
  Es werden der Konstruktor sowie die Getter von PointOnEdge getestet.
}{
  der Konstruktor nur positive EdgeIDs sowie Positionen zwischen 0 und 1 annimmt und die Getter die korrekten Werte zurückgeben.
}{bestanden}

\test{fileUtil}{FileUtil}{
  Es werden checkMapName und checkProfileName getestet.
}{
  checkMapName/checkProfileName zulässige Map-/Profilnamen akzeptiert und unzulässige ablehnt.
}{bestanden}
\end{itemize}
