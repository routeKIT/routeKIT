...
\bug{corruptIndex}{Korrupte Indexdatei}
    ...
    {Karten oder Profile überspringen, die \code{null} sind.}
    {behoben}
\end{itemize}
...
\bug{pointProjection}{Punkt-Projektion}
    {Die Projektion von eingegebenen Koordinaten auf die nächstliegenden Kanten ist oft schief.}
    {Die Projektion findet im Geokoordinatensystem statt und nicht nach der \gls{mercator} (also im \gls{smt}-Koordinatensystem).}
    {Für die Abstandsberechnung \gls{smt}-Koordinaten anstelle von Geokoordinaten benutzen.}
    {behoben}
...
\bug{testCLIHelp}{CLI-Hilfe Test}
    {Der Test für die Ausgabe des Hilfetextes für das Konsoleninterface schlägt auf Windows-Systemen
    mit einer \code{ArrayIndexOutOfBoundsException} fehl, obwohl er auf Linux-Systemen funktioniert.}
    {Die Ausgabe des Hilfetextes wird in ein Bytearray geschrieben. Dieses wird mit der Größe des Referenzstrings alloziert. Da auf Linux nur ein Byte für den Zeilenumbruch genutzt wird und auf Windows zwei, läuft das Array über.}
    {Den Referenzstring plattformabhängig mit den richtigen Zeilenumbrüchen erzeugen, damit er mit der Ausgabe exakt übereinstimmt.}
    {behoben}
\bug{deleteProfileWhileAddingToMap}{Karten- und Profilverwaltung: Inkonsistenz}
    ...
    Dieses Profil wird im Dialog für das Hinzufügen neuer Profile gelöscht.
...
\bug{currentProfileNotExisting}{Gelöschtes aktuelles Profil}
    ...
    Da das Profil nicht mehr in der Liste steht, wird automatisch ein anderes Profil gewählt, in diesem Fall ein Standardprofil.}
...
\bug{routeCalcRaceCondition}{Wettlauf bei der Routenberechnung}
    ...
    {Den aktuellen Berechnungsthread speichern und die berechnete Route nur noch abschicken, wenn der berechnende Thread der aktuelle ist.}
    {behoben}
\bug{zoomAndDrag}{Zoom und Ziehen}
    ...
    {Zoomen während einer Ziehgeste verhindern.}
    {behoben}
\bug{slowRender}{Vergröberung nötig}
    ...
    {Den Graph für die Anzeige vergröbern, sodass weniger Kanten gezeichnet werden müssen; siehe auch Abschnitt \ref{change:graphView}.}
    {behoben}
...
\bug{routeCalcKeineRoute}{Falscher Infotext während Routenberechnung}
    ...
    {Methode \code{isCalculated} zum \code{RouteModel} hinzufügen und verwenden; siehe auch Abschnitt \ref{change:routeCalcKeineRoute}.}
    {behoben}
\bug{centerMapProfileChange}{Kartenausschnitt zentriert bei Profilwechsel}
	{Der sichtbare Kartenausschnitt wird auf das Zentrum der Karte verschoben, obwohl man nur ein anderes Profil ausgewählt hat.}
	{Der Kartenausschnitt wird zentriert, sobald sich die aktuelle \code{ProfileMapCombination} ändert.}
	{Den Kartenausschnitt nur zentrieren, wenn sich die aktuelle Karte ändert.}
	{behoben}
\bug{textfieldsProfileMapChange}{Textfelder für Start und Ziel beim Profil-/Kartenwechsel}
	{Wird die Karte oder das Profil geändert, bleiben in den Textfeldern für Start und Ziel die alten Koordinaten stehen.}
	{Die Route wird zwar zurückgesetzt, aber die Textfelder werden nicht geleert.}
	{Die Textfelder leeren, sobald die Karte oder das Profil geändert werden.}
	{behoben}
\end{itemize}
...
\testfall
    ...
    {Ins Start- oder Zielfeld werden ungültige Koordinaten eingegeben.}
    {Das Feld färbt sich rot. Es wird keine Routenberechnung durchgeführt.}
\oitem{TF}{waydescription} Wegbeschreibung
\testfall
    {Das Start- oder das Zielfeld enthält gültige Koordinaten.}
    {Die fehlenden gültigen Koordinaten werden eingegeben.}
    {Die Markierungen auf der Karte wird gesetzt. Die Route zwischen Start- und Zielpunkt wird berechnet. Die Wegbeschreibung wird im Wegbeschreibungsfeld ausgegeben.}
...
\testfall
    ...
    {Der Hinzufügen-Button wird gedrückt. Ein Profil wird ausgewählt und die Wahl wird dreimal bestätigt.}
...
\testfall
    ...
    {Die Vorberechnung des Profils wird gelöscht.}
...
Die Tests /TF40/, /TF60/, /TF140/, /TF160/, /TF170/, /TF 180/, /TF190/, /TF260/, /TF270/, /TF280/, /TF300/, /TF320/ aus dem Pflichtenheft wurden manuell auf dem Commit \code{3c2b61df374375029a4fe853dce074f05488f9b6} am 07.\,03.\,2014 ausgeführt und bestanden.\\
...
\begin{longtable}{||c|c|c|c|c||}
Testnummer & bestanden & Startkoordinaten & Begründung\\ 
 & & Zielkoordinaten & \\ \hline \hline
\endfirsthead
...
Die Abdeckung des Quellcodes im \code{src}-Ordner (also ohne Testfälle) während des Commits \code{6d239a2ab7402c7a469b48299c42e8d56f547884} ist in Tabelle \ref{tab:coverageData} dargestellt.\\
...
\caption{Die Anweisungsüberdeckung des gesamten Testlaufs.}
...
\item\label{change:graphView} \code{GraphView} (\ref{BG:slowRender})
  ...
  \item Für kleinere Zoomstufen mit mehr sichtbaren Kanten wird hingegen \code{ReducedGraphView} verwendet, die den Graphen vergröbert.
    Zum Vergröbern des Graphen wird der Douglas-Peucker-Algorithmus verwendet. %TODO gls douglas peucker?
