...
\begin{description}
  \class{routeKIT}
        {Neue Klasse. Existiert ausschließlich, um den kurzen Aufruf
          \code{java routeKIT}
          zu ermöglichen.
        }
        {\none}
...
}
{
  ...
  \class{ProfileMapManager}
        {
          Neue Klasse. Der \code{ProfileMapManager} verwaltet alle \glspl{vorberechnung}: er lädt sie beim Programmstart von der Festplatte, löscht sie auf Anfrage, speichert sie und hält eine Liste aller \glspl{vorberechnung}.
          ...
          \method{startPrecalculation}
                 {
                   \param{reporter}{ProgressReporter}
                         {
                           Geändert: Der Parameter \code{combination} fällt weg; stattdessen wird immer die aktuelle \reftype{ProfileMapCombination} berechnet.
                           \refparam{MainController}{startPrecalculation}{reporter} wird über den Fortschritt der Vorberechnung informiert.
                         }
                 }{~}
        }
  ...
  \class{MapManagerController}{~}
        {\none}
        {
          ...
          \method{addProfile}
                 {\none}
                 {
                   Geändert: Die GUI übergibt nicht mehr das Profil als Parameter, stattdessen öffnet der \reftype{MapManagerController} selbst die Profilverwaltung, in welcher der Benutzer dann ein Profil auswählen kann.
                   In der Profilverwaltung können auch Profile und damit Vorberechnungen gelöscht werden, und diese Änderungen müssen dann auch in der \reftype{MapManagerView} wieder übernommen werden.
                   Diese Änderungen zu verfolgen, ist nicht Aufgabe der View, sondern des Controllers.
                 }
   ...
   \class{MapManagerController.MapManagementDiff}
          { 
           Neue Klasse. Innere Klasse von \reftype{MapManagerController}, welche die Änderungen, die der Benutzer getätigt hat, kapselt.
  ...
  \class{CLI}
        {
          Neue Klasse. Kapselt das \gls{cli}. Implementiert \reftype{ProgressListener}.
        }
        {\none}
        {
          \method{doImport}
...
}
{
  \class{PreCalculator}{Führt die Vorberechnung für die angegebene Kombination aus.}
         {\none}
         {
           \method{doPrecalculation}
                  {
                    \param{reporter}{ProgressReporter}{Wird zum Anzeigen des Fortschritts bei der \gls{arc}-Berechnung gebraucht.}
                  }
  ...
  \class{ArcFlagsCalculatorImpl}{Neue Klasse. Ersetzt die Klasse \code{ArcFlagsCalculator} aus dem Entwurf. Implementiert das Interface \reftype{ArcFlagsCalculator}}
        {\none}
        {
          \method{calculateArcFlags}
                 {
  ...
  \class{EdgeWeighter}{Geändert: Ein Interface für die Berechnung der Kantengewichte.}
        {\none}
        {
          \method{weightEdges}
                 {
                   \param{reporter}{ProgressReporter}{Wird zum Anzeigen des Fortschritts bei der Gewichte-Berechnung gebraucht.}
                 }
                 {Versieht den kantenbasierten Graphen mit Kantengewichten.}
        }
  \class{EdgeWeighterImpl}{Neue Klasse. Ersetzt die Klasse \code{EdgeWeighter} aus dem Entwurf. Implementiert das Interface \reftype{EdgeWeighter}}
        {\none}
        {
          \method{weightEdges}
                 {
  ...
  \class{TurnRestriction}{~}
        ...
        }
        {
          \method{TurnRestriction}
                 {\param{from}{int}{Die ID des OSM Wegs von dem ein Abbiegevorgang beschränkt ist.}
		\param{to}{MapEdge}{Eine Kante auf die ein Abbiegevorgang beschränkt ist.}
		}
                 {~}
...
\package{RouteCalculator}{}
{
  ...
  \class{FibonacciHeap}{Neue Klasse. Diese Klasse stellt einen Fibonacci-Heap dar, der vom \code{ArcFlagsDijkstra} und der Vorberechnung genutzt wird.}
        {\none}
        {
...
\package{Models}{}
{
  ...
  \class{ProgressReporter}
        {
          Neue Klasse. Dient zur Anzeige vom Fortschritt bei verschiedenen Aktionen, etwa der Vorberechnung, dem Benutzer.
  ...
  \class{ProgressListener}
        {
          Neues Interface. Wird benötigt um bei Änderungen den Fortschritts in \reftype{ProgressReporter} benachrichtigt zu werden.
        }
        {\none}
        {
          \method{startRoot}
  ...
  \class{CurrentCombinationListener}
        {
          Neues Interface. Dient der Benachrichtigung bei Änderung der aktuellen \reftype{ProfileMapCombination} (gehalten vom \reftype{ProfileMapManager}).
...
\package{Views}{Alle Views außer der \reftype{MapView} haben im Konstruktor einen neuen Parameter \code{Window parent}, der ein \code{Frame}, aus dem dieser Dialog geöffnet, wird darstellt.}
{
  \class{MainView}{~}
	...
	{Setzt in alle dafür vorgesehenen Felder den aktuellen Profil- und Kartennamen. Blockiert die \reftype{MapView}, falls es keine Vorberechnung für die aktuelle Kombination gibt und gibt durch \code{textMessage} die Meldung für den Benutzer aus.}
	\method{textMessage}
...
\class{MapManagerView}{~}
	...
	{\method{MapManagerView}
	{
		...
		\param{maps}{Set<StreetMap>}{Alle Karten, die importiert wurden. Der Benutzer kann in der Kartenverwaltung aus diesen Karten wählen.}
		\param{currentMapProfiles}{Set<Profile>}{Alle Profile, für die eine Vorberechnung für die aktuell ausgewählte Karte existiert.}
	}
  ...
  \class{Profile}{~}
        {\none}
        {
          \method{clone}[Profile]
                 {\none}
                 {
                   Erstellt eine Kopie eines Profils, welche nicht als Standardprofil gekennzeichnet ist.
                 }
        }
}
...
\package{Map}{}
{
  ...
  \class{Graph}{~}
        ...
        {Gibt die Kante in entgegengesetzter Richtung, oder -1, wenn sie nicht vorhanden ist (Erkennen von Einbahnstraßen). }
  ...
  \class{IntArraySet}{Neue Klasse. Ist ein \code{java.util.Set<Integer>} das die Integers aus einem Interval eines int-arrays darstellt.}
        {\none}
        {
        \method{IntArraySet}
        {\param{base}{int}{Der Index des ersten Elements}
        \param{count}{int}{Die Anzahl der Elemente}
        \param{data}{int[]}{Die zugrundeliegenden Daten}}
        {Erzeugt ein neues Set. Die Daten werden nicht kopiert sondern direkt wiedergegeben.}
        }
  \class{GraphIndex}{~}
        {\none}
        {
        \method{GraphIndex}
        {\param{maxType}{int}{Der Index des letzten StreetTypes, der noch angezeigt werden soll.}}
        {Der Graph entscheidet, welche Indices gebaut werden sollen und bei welchen 
        Zoomlevel welche Straßen verschwinden, sodass derselbe Index mehrfach benutzen kann.}
        }\textit{}
...
Linie. Dieses Verfahren kann verwendet werden, um die Koordinaten eines \reftype{PointOnEdge} zu bestimmen.}
  ...
  \class{FileUtil}
        {
          Neue Klasse. Erhält verschiedene statische Methoden, die Dateien und Pfade behandeln.
        }
        {\none}
        {
          \method{rmRf}
  ...
  \class{TimeUtil}
        {
          Neue Klasse. Enthält Methoden zur Zeitumrechnung.
        }
        {\none}
        {
  ...
  \class{Dummies}
        {
          Neue Klasse. Enthält drei Methoden, die eine \routeKIT-Installation erstellen, welche mehr oder weniger nützlich ist.
        }
        {\none}
        {
          \method{createDummies}
                 {
                   \param{rootDir}{File}{Das Wurzelverzeichnis der \routeKIT-Installation.}
                 }
                 {
                   Erstellt eine Dummy-Karte (fast leer) mit einer Dummy-Vorberechnung (Arc-Flags alle gesetzt).
                   Die älteste der drei Methoden, und jetzt nur noch nützlich, wenn man schnell irgendeine Vorberechnung braucht.
                 }
          \method{createInstall}
                 ...
                 {
                   Lädt eine Karte des Regierungsbezirks Karlsruhe von \code{geofabrik.de} herunter und führt mit dieser Karte eine Vorberechnung für das Profil „PKW (Standard)“ durch.
                   Dies erstellt eine vollständige und vollwertige Installation, kann allerdings eine Weile dauern.
                 }
          \method{downloadInstall}
                 {
                   \param{rootDir}{File}{Das Wurzelverzeichnis der \routeKIT-Installation.}
                 }
                 {
                   Lädt eine komplette, gezippte Installation von \code{dogcraft.de} herunter und extrahiert sie in \refparam{Dummies}{downloadInstall}{rootDir}.
                   Die Installation enthält eine Karte des Regierungsbezirks Karlsruhe sowie eine Karte von Baden-Württemberg, jeweils mit einer Vorberechnung für das Profil „PKW (Standard)“.
                 }
          \method{main}
                 {
                   \param{args}{String[]}{Wird ignoriert.}
                 }
                 {
                   Startet \refmethod{Dummies}{downloadInstall}.
                 }
        }
}
...
\begin{enumerate}
\item \bug{Für große Karten ist das Rendern der Kacheln zu langsam.}
{Es existieren zu viele Kanten auf einer Kachel.}
{Einen Vergröberunsalgorithmus realisieren.}
\item \bug{Die Anwendung braucht lange zum Starten.}
{\code{GraphIndex}e und Navigationsarrays im Graphen werden beim Starten neu aufgebaut.}
{Mehr Daten auf die Festplatte schreiben und beim Start laden.}
\item \bug{Nach dem Wechseln der Karte sind möglicherweise keine Kanten sichtbar.}
{Die neuen Kanten sind in einer anderen geografischen Region.}
{Einen Kartenmittelpunkt bestimmen und die Karte dort zentrieren.}
\item \bug{Nachricht „Sie haben ihr Ziel erreicht.“ wird bei jeder Wegbeschreibung ausgegeben.}
{Im Programmcode ist keine Bedingung vorhanden, die Nachricht nur bei existierender Beschreibung auszugeben.}
{Verfeinerung des Codes.}
\item \bug{Nachricht „Es wurde keine Route gefunden.“ wird niemals ausgeben.}
{Noch nicht im Code vorgesehen.}
{Spezialfall implementieren.}
\item \bug{Es ist möglich, so weit herauszuzoomen, dass die Karte nicht mehr sichtbar ist.}
{Die Zoomstufe wird nur statisch auf $[0,19]$ eingeschränkt.}
{Die Zoomstufe abhängig von der Größe der Karte einschränken.}
\end{enumerate}
...
Die Anzeige der Karte, für welche die Klasse \reftype{MapView} verantwortlich war, wurde vor den \reftype{TileRenderer} platziert, da dies als wichtiger für andere Teammitglieder eingestuft wurde.
