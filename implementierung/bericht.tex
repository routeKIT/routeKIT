...
\begin{enumerate}
  \bug{Für große Karten ist das Rendern der Kacheln zu langsam.}
      {Es existieren zu viele Kanten auf einer Kachel.}
      {Vergröberunsalgorithmus realisieren.}
  \bug{Die Anwendung braucht lange zum Starten.}
  ...
  \bug{Nach dem Wechseln der Karte sind möglicherweise keine Kanten sichtbar.}
      {Die neuen Kanten sind in einer anderen geografischen Region.}
      {Kartenmittelpunkt bestimmen und die Karte dort zentrieren.}
  \bug{Die Nachricht „Sie haben ihr Ziel erreicht.“ wird bei jeder Wegbeschreibung ausgegeben.}
      {Im Programmcode ist keine Bedingung vorhanden, die Nachricht nur bei existierender Beschreibung auszugeben.}
      {Code verfeinern.}
  \bug{Eine Nachricht „Es wurde keine Route gefunden.“ wird niemals ausgeben.}
      {Diese Nachricht ist noch nicht im Code vorgesehen.}
      {Spezialfall implementieren.}
  \bug{Es ist möglich, so weit herauszuzoomen, dass die Karte nicht mehr sichtbar ist.}
