...
\package{Precalculation}{}
{
  \class{OSMParser}{~}
        {\none}
        {
          \method{parseOSM}
                 {\none}
                 {Geändert: Der Parameter \code{name} wurde entfernt. Der Name der Karte wird erst später vom \refclass{MapImporter} gesetzt.}
        }
  \class{OSMWay}{~}
        {\none}
        {
  ...
  \class{MapEdge}{~}
        {
          \attr{targetNode}{Node}{Geändert: Speichert nun direkt den Zielknoten und nicht mehr dessen ID in der OSM-Datei. Dadurch werden unnötige Hashtabellenabfragen in \refclass{OSMParser} vermieden.}
  ...
  \class{Coordinates}{~}
        {\none}
        {
          \method{equals}[boolean]
                 {
                   \param{other}{Object}{Das zu vergleichende Objekt.}
                 }
                 {Bestimmt, ob zwei (Koordinaten-)Objekte gleich sind.}
