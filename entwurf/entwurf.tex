...
Das Muster \gls{mvc} umfasst die Pakete Controllers, Models, Views , wobei die Klassen im Controllers die Kommunikation zwischen den Klassen im Views und dem funktionalem Teil des Programms übernehmen.
Das Anfordern der Kacheln aus dem Paket MapDisplay erfolgt von der Klasse MapView direkt (ohne Einsatz von Controllern).
Die Klassen im Views sind für die Interraktion des Benutzers mit den Programm und die Darstellung der Daten aus dem Paket Models zuständig.
Die Bekanntgabe von Änderungen an relevanten Daten im Modell geschieht nach dem Entwurfsmuster „Beobachter“.
Durch den Einsatz vom Entwurfsmuster \gls{mvc} ist das Paket Views leich austauschbar.
Alle Klassen im Paket Controllers sind basieren auf dem Entwurfsmuster Singleton. 

Im Paket MapDisplay wird das Entwurfsmuster Decorator verwendet. % Vorteile davon.

Im Paket RouteCalculator wird das Entwurfsmuster Strategie eingesetz.
