...
\newcommand{\package}[3]{%
  ...
  \ifstrequal{#1}{BeispielPaket}{%
    % no figure for the sample package
  }{
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.9\pagewidth]{Package_#1}
      \caption{Das Paket #1}
      \label{fig:package_#1}
    \end{figure}
  }
  #2
  \begin{description}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Pakete}
...
Der grobe Entwurf der Anwendung wird durch das Muster \gls{mvc} vorgegeben, welches in den Paketen \refpackage{Models}, \refpackage{Views} und \refpackage{Controllers} umgesetzt ist.
...
\end{description}
Benutzeraktionen, die den Zustand ändern, sind etwa das Öffnen des Profilmanagers (siehe \abbildung{sequenz_profilVerwaltung}) oder die Eingabe eines Startpunkts;
...
Die Klassen \reftype{MainController}, \reftype{ProfileManager}, \reftype{MapManager} im Paket \refpackage{Controllers} basieren auf dem \gls{entwurfsmuster} \gls{einzelstueck}. % TODO nein? ManagerController werden immer wieder erstellt – Lucas (gemacht, stimmt hab mich vertan)
...
Dadurch wird die Funktionalität, dass ältere Zugriffe zwischengespeichert werden, von der Funktion, die Kacheln zu beschaffen, abgetrennt.
...
\begin{itemize}
\item Es findet keine Angabe darüber statt, wie lange eine Vorberechnung voraussichtlich dauern wird, da dies kaum abschätzbar ist; stattdessen wird einfach darauf hingewiesen, dass die Vorberechnung mehrere Stunden dauern kann. (Pflichtenheft Abschnitte 9.2, 9.5)
\end{itemize}
...
\package{Controllers}{
 ...
 Der \reftype{MainController} nimmt Benutzeraktionen von der \reftype{MainView} und \reftype{MapView} entgegen und reagiert darauf.
 
 Der \reftype{MainController} erstellt den \reftype{ProfileManagerController} bzw. den \reftype{MapManagerController}, wenn der Benutzer das Fenster der \gls{profil}- bzw. Kartenverwaltung öffnet.
  ...
  \class{MainController}
        ...
        {\none}
        {
          \method{MainController}
                 {\none}
                 {
                   Konstruktor: Erstellt den Controller, lädt \glspl{profil}, die Namen der \glspl{karte} und die aktuelle \gls{karte} vollständig und erstellt dann die \reftype{MainView}.
          ...
          \method{setStartAndDestinationPoint}
                 {
                   \param{start}{Coordinates}{Die Koordinaten des neuen Startpunkts.}
                   \param{destination}{Coordinates}{Die Koordinaten des neuen Zielpunkts.}
                 }
                 {
                   ...
                   Die gleichen Aktionen wie für \refmethod{MainController}{setStartPoint} und \refmethod{MainController}{setDestinationPoint} werden ausgeführt, nur nicht doppelt.
                 }
          ...
          \method{startPrecalculation}
                 ...
                 {
                   Ruft in einem neuen WorkerThread \refmethod{PreCalculator}{doPrecalculation} auf, falls keine \gls{vorberechnung} für diese Kombination aus \gls{profil} und \gls{karte} existiert. Sperrt währenddessen die \reftype{MainView}.
                 }
          ...
          \method{main}
	             ...
	             {(statisch) Hauptmethode des Programms. Erzeugt einen \reftype{MainController}.}
        }
  ...
  \class{ProfileManagerController}
        ...
        {\none}
        {
          ...
          \method{getDeletionTime}[int]
                 {\none}
                 {
                   Gibt zurück, wie viel Zeit die \glspl{vorberechnung} benötigt haben, die durch die aktuell erfassten Änderungen gelöscht werden.
                   Die Dauer wird in Millisekunden zurückgegeben.
                 }
  ...
  \class{MapManagerController}
        ...
        {\none}
        {
          ...
          \method{saveAllChanges}
                 ...
                 {
                   Führt alle vom Benutzer vorgenommenen Änderungen aus.
                   Dazu gehören das Importieren und Löschen von \glspl{karte} sowie das Hinzufügen oder Löschen von \glslink{profil}{Profilen} je \gls{karte} (Löscht also \glspl{vorberechnung} oder erzeugt neue).
                 }
        }
  ...
  \class{MapManager}
        ...
        {\none}
        {
          ...
          \method{deleteMap}
                 ...
                 {
                   Löscht die ausgewählte \gls{karte} aus der internen Liste und von der Festplatte (die zwei Graphen, Daten über Beschränkungen und alle \glspl{vorberechnung}).
                 }
...
\package{Precalculation}{
  ...
  Beim Importieren einer \gls{karte} werden ein knotenbasierter und ein kantenbasierter Graph aufgebaut und der kantenbasierte Graph partitioniert.
  Bei der \gls{vorberechnung} wird der kantenbasierte Graph vom \reftype{EdgeWeighter} gewichtet und danach vom \reftype{ArcFlagsCalculator} mit \gls{arc} versehen.
  % TODO kurze erklärung, wie die vorberechnung läuft (gemacht)
  
  Ein beispielhafter Ablauf der Vorberechnung ist in \abbildung{sequenz_vorberechnung} zu sehen.
}
{
  \class{MapImporter}
  ...
  \class{OSMParser}
        {Stellt die Funktionalität zum Parsen einer \gls{osm}-Datei bereit. Dafür werden zur temporären Repräsentation des Graphen im Speicher die Klassen \reftype{OSMWay}, \reftype{MapEdge} und \reftype{TurnRestriction} verwendet.}
        {\none}
        {
  ...
  \class{OSMWay}
        {Stellt einen Weg aus der OSM-Datei dar. Dies ist nur eine vom \reftype{OSMParser} verwendete Hilfsklasse.}
        {\none}
        {
          \method{OSMWay}
                 {
                   \param{props}{Map<String, String>}{Eine Liste von OSM-Tags.}
                 }
          ...
          \method{getEdgeProperties}[EdgeProperties]
                 {\none}
                 {Liefert ein \reftype{EdgeProperties}-Objekt mit den Eigenschaften des Wegs.}
          \method{getRestriction}[Restriction]
                 {\none}
                 {Liefert ein \reftype{Restriction}-Objekt mit der/den Beschränkung(en) des Wegs oder \code{null}, falls nicht vorhanden.}
        }
  \class{MapEdge}
        {Stellt eine Kante im Straßengraphen dar. Dies ist nur eine vom \reftype{OSMParser} verwendete Zwischendarstellung.}
        {
          \attr{targetNode}{int}{Die ID des Zielknotens in der OSM-Datei.}
        }
        {
          \method{MapEdge}
                 {
                   \param{target}{int}{Die ID des Zielknotens.}
                   \param{way}{OSMWay}{Der zur Kante gehörige OSM-Weg.}
                 }
  ...
  \class{TurnRestriction}
        ...
        {
          \attr{onlyAllowedTurn}{boolean}{Gibt an, ob es sich um die ausschließlich erlaubte Abbiegemöglichkeit handelt (\code{true}) oder um ein Abbiegeverbot (\code{false}).}
        }
        {
          \method{TurnRestriction}
                 {
                   \param{onlyAllowedTurn}{boolean}{Der Wert des gleichnamigen Attributs.}
                 }
...
\package{RouteCalculator}{
 ...
 Die Klasse \reftype{ArcFlagsDijkstra}, die das Interface \reftype{RouteCalculator} implementiert, erstellt eine \reftype{Route}. Zu dieser \reftype{Route} erstellt \reftype{RouteDescriptionGenerator} eine \reftype{RouteDescription}.
 
 Ein beispielhafter Ablauf der Routenberechnung ist in \abbildung{sequenz_routeCalculation} zu sehen.
}
{
  ...
  \class{Route}
        ...
        }
        {
          \method{Route}
                 {
                   \param{data}{ProfileMapCombination}{Die Karte, auf dem die Route berechnet wurde.}
                   \param{start}{PointOnEdge}{Der Startpunkt der Route.}
                   \param{destination}{PointOnEdge}{Der Zielpunkt der Route.}
                   \param{turns}{List<Turn>}{Die Liste der Abbiegevorgänge der Route.}
                 }
                 {Konstruktor: Erzeugt ein neues Routen-Objekt mit den angegebenen Attributen.}
  ...
  \class{RouteDescription}
        ...
        {\none}
        {
          \method{RouteDescription}
                 {
                   \param{route}{Route}{Die Route, die die Wegbeschreibung beschreibt.}
                   \param{instructions}{List<TurnInstruction>}{Eine Liste von Abbiegeanweisungen.}
                 }
  ...
  \class{TurnInstruction}
        ...
        {
          \attr{turn}{Turn}{Der Abbiegevorgang, den die Anweisung beschreibt.}
        }
        {
          \method{TurnInstruction}
                 {
                   \param{turn}{Turn}{Der Abbiegevorgang, den die Anweisung beschreibt.}
                   \param{text}{String}{Der Text der Abbiegeanweisung.}
                 }
                 {Konstruktor: Erzeugt ein neues Objekt mit den angegebenen Attributen.}
  ...
  \class{TileCache}
        ...
        {\none}
        {
          \method{TileCache}
                 ...
                 {Konstruktor: Erstellt einen neuen Cache für die angegebene \reftype{TileSource}.}
          \method{renderTile}[BufferedImage]
                 {
  ...
  \class{ProfileMapCombination}
        {Eine Kombination aus einem \reftype{Profile}[Profil] und einer \reftype{StreetMap}[Karte].}
        {
          ...
          \attr{calculationTime}{int}
               {
                  Die Zeit, die für die Vorberechnung dieser Kombination benötigt wurde, in Millisekunden.
  ...
  \class{MapView}
        ...
        {\none}
        {
          \method{MapView}
                 ...
                 {
                   Konstruktor: Erzeugt eine neue \reftype{MapView}. Die angegebene \reftype{TileSource} wird zum \Gls{rendern} verwendet.
  ...
  \class{HistoryView}
        ...
        {\none}
        {
          \method{HistoryView}
                 ...
                 {
                   Konstruktor: Erstellt eine HistoryView für den angegebenen \gls{verlauf}.
  ...
  \class{Profile}
        ...
        }
        {
          ...
          \method{load}[Profile]
                 ...
                 {(statisch) Lädt ein \gls{profil} aus der angegebenen Datei und gibt es zurück.}
        }
  \enum{VehicleType}
       {
         Ein Fahrzeugtyp.
...
\package{Map}{
  ...
  \begin{description}
  \item[\reftype{Graph}] ist ein regulärer, knotenbasierter Graph, gespeichert als Adjazenzfeld. Aus ihm werden geometrische Datenstrukturen \reftype{GraphIndex} für verschiedene Zoomstufen erzeugt. Er wird zum \Gls{rendern} der \gls{karte} verwendet.
...
}
{
  \class{StreetMap}
        ...
        }
        {
          \method{StreetMap}
                 {
                   \param{graph}{Graph}{Der Kartengraph.}
                   \param{edgeBasedGraph}{EdgeBasedGraph}{Der kantenbasierte Graph.}
                 }
  ...
  \class{GraphIndex}
        ...
        {\none}
        {
	      \method{GraphIndex}
		         ...
		         {Konstruktor: Erzeugt die Datenstruktur für den gegebenen Graph und die angegebene Zoomstufe.}	
  ...
  \class{EdgeProperties}
        ...
        {
          \attr{name}{String}{Der Name der Straße oder \code{null}, falls nicht vorhanden.}
          \attr{roadRef}{String}{Die Nummer der Straße oder \code{null}, falls nicht vorhanden.}
        }
        {
          \method{EdgeProperties}
                 {
                   \param{type}{HighwayType}{Der Straßentyp.}
                   \param{name}{String}{Der Wert für \refattr{EdgeProperties}{name}.}
                   \param{roadRef}{String}{Der Wert für \refattr{EdgeProperties}{roadRef}.}
                   \param{maxSpeed}{int}{Die zulässige Höchstgeschwindigkeit für diese Kante oder \code{0}, falls nicht festgelegt.}
                 }
  ...
  \class{EdgeBasedGraph}
        ...
        {\none}
        {
          \method{EdgeBasedGraph}
                 {
                   \param{edges}{Edge[]}{Das Knoten-Array (Kanten im Straßengraph) des Adjazenzfelds.}
                   \param{turns}{turn[]}{Das Kanten-Array (Abbiegemöglichkeiten) des Adjazenzfelds.}
                   \param{turnTypes}{TurnType[]}{Die Typen der Abbiegemöglichkeiten.}
                   \param{restrictions}{Map<Turn, Restriction>}{Die Beschränkungen der Abbiegemöglichkeiten.}
                 }
  ...
  \class{Restriction}
        {Abstrakte Klasse zur Repräsentation unterschiedlicher Beschränkungen für Straßen oder Abbiegemöglichkeiten. Die einzelnen Unterklassen sind Multitons, um nicht unnötig Speicherplatz für mehrere gleiche Objekte zu verbrauchen.}
        {\none}
        {
          \method{allows}[boolean]
                 {
                   \param{profile}{Profile}{Das verwendete \gls{profil}.}
                 }
                 {Bestimmt, ob es die Beschränkung erlaubt, unter dem angegebenen \gls{profil} eine Straße oder Abbiegemöglichkeit zu nutzen.}
        }
  \class{VehicleTypeRestriction}
        {Repräsentiert eine Beschränkung der Benutzung durch einen bestimmten Fahrzeugtyp.}
        {\none}
        {
          \method{getInstance}[Restriction]
                 {
                   \param{type}{VehicleType}{Der Fahrzeugtyp.}
                 }
                 {(statisch) Gibt eine Instanz dieser Klasse für den angegebenen Fahrzeugtyp zurück.}
        }
  \class{WeightRestriction}
        ...
        {\none}
        {
          \method{getInstance}[Restriction]
                 {
                   \param{weight}{int}{Das maximale Fahrzeuggewicht.}
                 }
                 {(statisch) Gibt eine Instanz dieser Klasse für den angegebenen Wert zurück.}
        }
  \class{WidthRestriction}
        {Repräsentiert eine Beschränkung der Fahrzeugbreite.}
        {\none}
        {
          \method{getInstance}[Restriction]
                 {
                   \param{width}{int}{Die maximale Fahrzeugbreite.}
                 }
                 {(statisch) Gibt eine Instanz dieser Klasse für den angegebenen Wert zurück.}
        }
  \class{HeightRestriction}
        ...
        {\none}
        {
          \method{getInstance}[Restriction]
                 {
                   \param{height}{int}{Die maximale Fahrzeughöhe.}
                 }
                 {(statisch) Gibt eine Instanz dieser Klasse für den angegebenen Wert zurück.}
        }
  \class{MultipleRestrictions}
        {Repräsentiert eine Kombination aus mehreren Beschränkungen.}
        {\none}
        {
          \method{getInstance}[Restriction]
                 {
                   \param{restrictions}{Set<Restriction>}{Eine Menge von Beschränkungen.}
                 }
                 {(statisch) Gibt eine Instanz dieser Klasse für den angegebenen Fahrzeugtyp zurück.}
        }
}
  ...
  \class{History}
        ...
        {\none}
        {
          ...
          \method{getEntries}[List<HistoryEntry>]
                 {\none}
                 {Gibt alle Einträge des \glslink{verlauf}{Verlaufs} zurück.}
          \method{save}
                 {
          ...
          \method{load}[History]
                 ...
                 {(statisch) Lädt einen \gls{verlauf} aus der angegebenen Datei und gibt ihn zurück.}
        }
  \class{HistoryEntry}
...
}
{
  \class{Coordinates}
        ...
        }
        {
          ...
          \method{angleBetween}[float]
                 {
                   \param{coords1}{Coordinates}{Die Koordinaten des ersten Punkts.}
                   \param{coords2}{Coordinates}{Die Koordinaten des zweiten Punkts.}
                 }
                 {Berechnet den zwischen einer Linie von diesem zum ersten und einer Linie von diesem zum zweiten Punkt eingeschlossenen Winkel.}
          ...
          \method{fromSmt}[Coordinates]
                 ...
                 {(statisch) Rechnet \gls{smt}-Koordinaten in Koordinaten um.}
        }
  \class{PointOnEdge}
...
\abbildung{sequenz_start} zeigt den Programmstart.
\def\currentclass{__BOGUS__} % quick and dirty hack to make \refmethod work outside of a class definition
Zunächst wird der \reftype{MainController} erstellt (Konstruktor \refmethod{MainController}{MainController});
dieser erstellt wiederum die restlichen Komponenten des Programms.
\begin{figure}[H]
\centering
\includegraphics[angle=90, width=0.8\textwidth]{StartSequenz}
\caption{Start des Programms}
\label{fig:sequenz_start}
\end{figure}
Beteiligte Klassen: \reftype{RouteKit}, \reftype{MainController}, \reftype{MapManager}, \reftype{ProfileManager}, \reftype{ProfileMapCombination}, \reftype{RouteModel}, \reftype{MainView}, \reftype{TileCache}, \reftype{MapView}

\subsection{Rendern}
\abbildung{sequenz_rendern} zeigt, wie die \reftype{MapView} eine Kartenkachel vom \reftype{TileCache} anfordert.
...
\abbildung{sequenz_routeCalculation} zeigt den Ablauf der Routenberechnung, wenn noch kein Start- und Zielpunkt gesetzt sind.
...
\end{figure}
Beteiligte Klassen: \reftype{MainView}, \reftype{MapView}, \reftype{MainController}, \reftype{RouteModel}, \reftype{RouteCalculator}, \reftype{RouteDescription}, \reftype{Route}, \reftype{RouteDescription}

\subsection{Profilverwaltung}
\abbildung{sequenz_profilVerwaltung} zeigt den Ablauf einer Profilverwaltung, bei der der Benutzer ein neues Profil „Smart“ anlegt, es 500\,kg leichter macht und dann ein Profil „Mercedes“ löscht.
\begin{figure}[H]
\centering
...
\abbildung{sequenz_vorberechnung} zeigt den Ablauf der Vorberechnung.
\begin{figure}[H]
\centering
...
\end{figure}
Beteiligte Klassen: \reftype{MainController}, \reftype{PreCalculator}, \reftype{EdgeWeighter}, \reftype{ArcFlagsCalculator}, \reftype{ProfileMapCombination}
