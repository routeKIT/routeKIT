...
Beim Entwurf der Anwendung \routeKIT wurden Entwurfsmuster eingesetzt. Das Muster \gls{mvc} umfasst die Pakete Controllers, Models, Views , wobei die Klassen im Controllers die Kommunikation zwischen den Klassen im Views und dem funktionalem Teil des Programms übernehmen. Das Anfordern der Kacheln aus dem Paket MapDisplay erfolgt von der Klasse MapView direkt (ohne Einsatz von Controllern). Die Klassen im Views sind für die Interraktion des Benutzers mit den Programm und die Darstellung der Daten aus dem Paket Models zuständig. Die Bekanntgabe von Änderungen an relevanten Daten im Modell geschieht nach dem Entwurfsmuster „Beobachter“. Durch den Einsatz vom Entwurfsmuster \gls{mvc} ist das Paket Views leich austauschbar. Alle Klassen im Paket Controllers sind basieren auf dem Entwurfsmuster Singleton. 

Im Paket MapDisplay wird das Entwurfsmuster Decorator verwendet. % Vorteile davon.

Im Paket RouteCalculator wird das Entwurfsmuster Strategie eingesetz. Der Algorithmus zur Berechnung der Route ist in eine eigene Klasse \reftype{ArcFlagsDijkstra} gekapselt und somit leicht zu ersetzen.  
Im Paket Precalculation wird ebenfalls dieses Muster verwendet. Somit wird ermöglicht das Programm zum Partitionieren eines Graphen auszutauschen.
...
\package{Controllers}{
 ...
 Der \reftype{Controller} erstellt den \reftype{ProfileManagerController} bzw. den \reftype{MapManagerController}, wenn der Benutzer das Fenster der Profil- bzw. Kartenverwaltung öffnet.
 Diese sind für \reftype{ProfileManagerView} bzw. die \reftype{MapManagerView} verantwortlich.

 Der \reftype{ProfileManager} und der \reftype{MapManager} sind für das Speichern und Laden der Profil- oder Kartendaten zuständig.
  ...
  \class{Controller}
        ...
        {\none}
        {
          \method{Controller}
                 {\none}
                 {
                   Erstellt den Controller, lädt Profile und Karten und erstellt dann die \reftype{MainView}.
		  ...
		  \method{onPrecalculationFinished}
                 {\none}
                 {
                   Wird aufgerufen, wenn eine Vorberechnung abgeschlossen ist. Reaktiviert Benutzerinteraktion mit der mainView.
                 }
	  ...
	  Ein Beispiel für die Kommunikation zwischen den beiden Klassen ist in \abbildung{sequenz_profilVerwaltung} zu sehen.
        ...
        {\none}
        {
          \method{saveTemporaryProfile}
                 {
                   \param{profile}{Profile}{Das temporäre Profil mit den aktuell eingegebenen Werten.}
                 }
                 {
                   Speichert die Werte des temporären Profils.
          ...
          \method{changeTemporaryProfile}
                 {
                   \param{name}{String}{Der Name des neuen Profils.}
                 }
                 {
                   Wechselt zu dem temporären Profil mit dem angegebenen Namen.
                   Falls noch kein Profil mit diesem Namen existiert, wird es als Kopie des aktuellen Profils erstellt.
          ...
          \method{getDeletionTime}[int]
                 {\none}
                 {
                   Gibt zurück, wie lange die Vorberechnungen benötigten, die durch die aktuell erfassten Änderungen gelöscht werden.
...
\package{Precalculation}{
  Dieses Paket enthält alle Klassen, die an der \gls{vorberechnung} beteiligt sind oder für den Import einer Karte benötigt werden.
  Beim Importieren einer Karte werden ein knotenbasierter und ein kantenbasierter Graph aufgebaut und der kantenbasierte Graph wird partitioniert.
  Bei der \gls{vorberechnung} werden erst die Gewichte der Kanten vom \reftype{EdgeWeighter} berechnet und danach die \gls{arc} vom \reftype{ArcFlagsCalculator}.
  ...
  \class{MapImporter}
        ...
        {\none}
        {
          \method{importMap}[StreetMap]
                 {
                   \param{file}{File}{Die OSM-Datei, aus der die Kartendaten importiert werden sollen.}
  ...
  \class{OSMParser}
        ...
        {\none}
        {
          \method{parseOSM}[StreetMap]
                 {
                   \param{file}{File}{Die OSM-Datei, die eingelesen werden soll.}
                   \param{name}{String}{Der Name der neuen \reftype{StreetMap}.}
                 }
...
\package{RouteCalculator}{
 Dieses Paket enthält alle Klassen, die zur Routenberechnung unter Berücksichtigung von \gls{arc} und Erzeugung der Wegbeschreibung benötigt werden.
}
{
...
{  \class{ProfileMapCombination}
        {Eine Kombination aus einem \reftype{Profile}[Profil] und einer \reftype{StreetMap}[Karte].}
        {
          ...
          \attr{calculationTime}{int}{
            Die Zeit, die benötigt wurde, um diese Kombination vorzuberechnen.
            Die Zeit wird in Millisekunden angegeben.
            Für Kombinationen ohne Vorberechnung ist dieser Wert \code{0}.
          }
        }
        {
