...
\newcommand{\attr}[3]{
  \item[\namedlabel{A:\currentclass.#1}{#1}] (\texttt{#2}) #3.
}
% Argumente:
% 1. Name
% 2. Typ
...
% 4. Beschreibung
\newcommand{\method}[4]{
  \item[\namedlabel{M:\currentclass.#1}{#1}] (\texttt{#2}) #4\\
    Parameter:
    \begin{description}
...
% 3. kurze Beschreibung, ohne Punkt am Ende
\newcommand{\param}[3]{
  \item[\namedlabel{P:\currentclass.\currentmethod.#1}{#1}] (\texttt{#2}) #3.
}
...
\class{Map}
       {Eine Karte.}
       {
         ...
         \attr{renderingData}{RenderingData}{Die Daten, die zum Rendern der Karte benötigt werden}
       }
       {\keine}
\class{ProfileAndMap}
       {Eine Kombination aus einem \refclass{Profile}[Profil] und einer \refclass{Map}[Karte]. Siehe auch \refclass{MapData}.}
       {
