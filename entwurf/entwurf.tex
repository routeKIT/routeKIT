...
\package{RouteCalculator}{
  ...
  \class{TurnInstruction}
        ...
        {\keine} % TODO
  \class{Arcflags}
        {Enthält die Arcflags für den vorberechneten Graphen.}
        {\keine} % ?
        {
          \method{getFlags}[int]
                 {
...
\package{Precalculation}{
  \class{PreCalculationScheduler}
  		        {Veranlasst die Vorberechnung für Kombinationen aus \refclass{Profile}[Profil] und \refclass{Map}[Karte].}
  		        ...
  		        {\method{scheduleCalculation}
  		        	{
  		        	\param{comb}{ProfileMapCombination}{Die Kombination aus \refclass{Profile}[Profil] und \refclass{Map}[Karte], für die die Vorberechnung durchgeführt werden soll.}
  		        	}
  		        	{Fügt die \refclass{ProfileMapCombination}[ProfileMapCombination] zur späteren Vorberechnung zur Queue hinzu.}
  		        }
  		        
  	\class{PreCalculator}
        {Führt die Vorberechnung für eine Kombination aus \refclass{Profile}[Profil] und \refclass{Map}[Karte] durch.}
        {\keine}
        {
          \method{doPrecalculation}
                 ...
                 {Führt die Vorberechnung für die gegebene Kombination aus \refclass{Profile}[Profil] und \refclass{Map}[Karte] durch. Dabei werden ein \refclass{EdgeWeighter}[EdgeWeighter] und ein \refclass{ArcFlagsCalculator}[ArcFlagsCalculator] aufgerufen.}
        }
  \class{MapImporter}
  ...
  \class{OSMParser}
        ...
        {\keine}
        {
          \method{parseOSM}[Graph]
                 ...
                 {Importiert eine OSM-Karte in eine verlinkte Datenstruktur \refclass{MapNode}}
        }
  \class{MapNode}
        ...
        {\keine}
        {
          ...
          \method{getNeighbours}[List<MapNode>]
                 {\keine}
                 {Gibt eine Liste mit allen Knoten, zu denen eine Kante von diesem Knoten existiert, zurück.}
        }
        ...
        {Berechnet Arcflags für einen partitionierten, gewichteten Graphen.}
        {\keine}
        {\method{calculateArcFlags}[ArcFlags]
            ...
            {Berechnet Arcflags für den gegebenen Graphen mit den gegebenen Partitionen und Kantengewichten.}
		}
...
\package{Controllers}{
    \class{ProfileManager}
        {Verwaltet die Profile. Hat intern eine Liste von vorhandenen Profilen.}
        {\keine}
        {
                \method{createProfile}
                {\keine }
                {Erstellt ein neues Profile-Objekt, nimmt es in die Liste auf und setzt es als aktuell ausgewähltes Profil.}
                ...
                {\keine }
                {Löscht das ausgewählte Profil aus der internen Liste und die Datei.}
         }
  \class{MapManager}
        {Verwaltet die Kartendaten. Hat intern eine Liste von vorhandenen Kartendaten.}
        {\keine}
        {
                \method{importMap}
                {\keine }
                {Nach Angabe des korrekten Pfads wird \refmethod{MapImporter}{import}[import] aufgerufen.}
         
                 \method{deleteMap}
                {\keine }
                {Löscht die ausgewählten Karte aus der internen Liste. Löscht die OSM-Datei, den Graphen und alle vorhandenen Vorberechnungen der ausgewählten Karte.}
         }
 \class{Controller}
        ...
        {\keine}
        {
                ...
                {Wählt das aktuelle Profil aus.}

                \method{selectMap}
                {
	            \param{map}{Map}{Die aktuelle Karte.} 
	      }
                {Wählt die aktuelle Karte aus.}
                ...
                {Ruft \refmethod{PreCalculationScheduler}{scheduleCalculation}[scheduleCalculation] auf, falls keine Vorberechnung für diese Kombination aus Profil und Karte exestiert.}
         }
