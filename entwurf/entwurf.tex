...
\newcommand{\class}[4]{
  ...
  \item[\namedlabel{\@classlabel{#1}}{\code{#1}}] #2
    \ifstrequal{#3#4}{\none\none}{
      % \none attributes and \none methods, skip
    }{
      \begin{description}
...
\newcommand{\attr}[3]{
  \item[\namedlabel{\@attrlabel{\currentclass}{#1}}{\code{#1}}] #3 Typ:~\refclass{#2}
}
% Argumente:
% 1. Name
...
\NewDocumentCommand{\method}{m o m +m}{ % the ‘+m’ means that parameter 4 can contain several paragraphs, see http://tex.stackexchange.com/questions/64328/blank-lines-break-xparse-or-ifthen-macro
  ...
  \item[\namedlabel{\@methodlabel{\currentclass}{#1}}{\code{#1}}] #4\\
    ...
    \IfValueTF{#2} % if the optional “type” argument is given...
              {Rückgabetyp:~\refclass{#2}} % then show the return type...
              {} % else don’t.
...
% 3. kurze Beschreibung
\newcommand{\param}[3]{
  \item[\namedlabel{\@paramlabel{\currentclass}{\currentmethod}{#1}}{\code{#1}}] #3 Typ:~\refclass{#2}
}
% Argumente:
% 1. Name
% 2. Beschreibung
...
\newcommand{\enum}[3]{
  ...
  \item[\namedlabel{\@enumlabel{#1}}{\code{#1}}] #2
...
\NewDocumentCommand{\refclass}{mO{#1}}{%
  \ifhaslabel{\@classlabel{#1}}{% if the label exists...
    \hyperref[\@classlabel{#1}]{\code{#2}}}{% then reference it...
...
% Argumente:
% 1. Enum
% 2. (optional) Text – default: Enum
\NewDocumentCommand{\refenum}{mO{#1}}{%
  \ifhaslabel{\@enumlabel{#1}}{% if the label exists...
    \hyperref[\@enumlabel{#1}]{\code{#2}}}{% then reference it...
    \code{#2}}\xspace% else just print the text.
}
% Argumente:
% 1. Enum
