...
Beim Entwurf der Anwendung \routeKIT wurden Entwurfsmuster eingesetzt.
...
Durch den Einsatz vom Entwurfsmuster \gls{mvc} ist das Paket \refpackage{Views} leicht austauschbar.
Alle Klassen im Paket \refpackage{Controllers} basieren auf dem Entwurfsmuster „Einzelstück“. % TODO nein? ManagerController werden immer wieder erstellt – Lucas

Im Paket \refpackage{MapDisplay} wird das Entwurfsmuster „Dekorierer“ verwendet.
...
Im Paket \refpackage{RouteCalculator} wird das Entwurfsmuster „Strategie“ eingesetzt.
Der Algorithmus zur Berechnung der Route ist in eine eigene Klasse \reftype{ArcFlagsDijkstra} gekapselt und somit leicht zu ersetzen.  
...
\package{Map}{
  ...
  \begin{description}
  \item[\reftype{Graph}] ist ein regulärer, knotenbasierter Graph, gespeichert als Adjazenzfeld. Aus ihm wirden dynamisch geometrische Datenstrukturen \reftype{GraphIndex} für verschiedene Zoomstufen erzeugt. Er wird zum \Gls{rendern} der Karte verwendet.
  ...
  \class{GraphIndex}
        ...
        {\none}
        {
	      ...
	      \method{getEdgesInRectangle} [Edge[]]
		         ...
		         {
                   Bestimmt alle Kanten innerhalb eines rechteckigen Kartenausschnitts, der durch \refparam{GraphIndex}{getEdgesIn}{leftTop} und \refparam{GraphIndex}{getEdgesIn}{rightBottom} festgelegt ist.
                 }
...
}
{
  \class{Coordinates}
        ...
        {
          \attr{lat}{float}{Der Breitengrad des Koordinatenpaars.}
          \attr{lon}{float}{Der Längengrad des Koordinatenpaars.}
        }
        {
          \method{Coordinates}
                 {
