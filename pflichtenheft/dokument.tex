...
\oitem{WF}{saveHist}
Hinzufügen einer Routenanfrage zur History
\oitem{WF}{getHist}
Start- und Zielpunkt aus der History auswählen
\end{description}
...
% Betreuer2: History,
%TODO  Menu, Profil/Kartenknopf ins Label integrieren
...
\item History-Button: erlaubt Zugriff auf die \gls{history}, siehe Abbildung \ref{fig:mockuphistory}
...
%TODO "Expertenfeature", "dauert ca. 10Stunden". auf den Knopf drauf
\begin{figure}[H]
\centering
...
%TODO Welche Art von Interaktion ist noch möglich? Kann die Karte trotzdem noch verschoben/gezoomt werden? Oder steht das schon irgendwo?
...
\item Unterpunkt "Über...": Zeigt Informationsfenster, siehe \ref{fig:mockupabout} % TODO der mockup fehlt noch!
...
\subsection{Dialog: History}
\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{History}
\caption{History}
\label{fig:mockuphistory}
\end{figure}
\begin{enumerate}
\item History-Liste: Eine Liste aller \gls{history}-Einträge, mit dem neuesten ganz oben.
\item Scrollbalken: Um durch die History-Liste zu scrollen.
\item OK-Button: Setzt den aktuellen Start- und Zielpunkt nach dem ausgewählten History-Eintrag und schließt den Dialog.
\item Abbrechen-Button: Schließt den Dialog.
\end{enumerate}

Im History-Dialog kann der Benutzer frühere Anfragen einsehen und wiederverwenden. Ein einzelner Linksklick auf einen Eintrag wählt diesen Eintrag aus, ein doppelter Linksklick hat den gleichen Effekt wie ein einfacher Linksklick und ein Linksklick auf den OK-Button.
% Fabian
