...
%TODO Programm/Software => routeKIT wo angebracht
...
% Betreuer2: Aufteilen, Genauer TODO: sichherer Formulieren (gemacht)
...
Erzeugung einer textuellen Weganweisung ("dem Straßenverlauf folgen", "in 2km ...", etc.) aus einem Routenstück
...
Das GUI Element, das Kartenkacheln dynamisch rendern lässt, cachet und skaliert an der richtigen Position im Fenster anzeigt. Realisiert u.a \ref{MK:karteAnzeigen}, \ref{MK:karteVerschieben}, \ref{MK:karteZoomen}, \ref{MK:startZielZeigen}, \ref{MK:routeAnzeigen}
...
\begin{enumerate}
\item Menüleiste:\\
bietet Zugriff zum Menü (siehe \abbildung{mockupscreenshotmenu})
\item Profil-Button:\\
starte Profilverwaltung (siehe \abbildung{mockupprofilverwaltung}); daneben wird das aktuell ausgewählte \gls{profil} angezeigt
\item Karte-Button:\\
Wähle eine Karte (siehe \abbildung{mockupkartenverwaltung}); daneben wird die aktuell ausgewählte Karte angezeigt
\item ausgewählter Start- und Zielkoordinaten: mit \ref{WK:koordinaten} ist hier auch Eingabe möglich; sonst nur Anzeige
...
\item Kartenansicht: die aktuelle Karte aus selbstgerenderten oder \glspl{osmkachel}
\item Zoom-Buttons: \texttt{+} zum Hineinzoomen; \texttt{-} zum Herauszoomen
\item ausgewählter Startpunkt
\item ausgewählter Zielpunkt
\item berechnete \gls{route}
\item Kontextmenü: „Start hier“ setzt den Startpunkt; „Ziel hier“ den Zielpunkt
\end{enumerate}
...
Um der Benutzer vor der langen, aufwändigen Vorberechnung zu warnen erscheint vorher ein warnender Dialog mit:\\
...
\item Unterpunkt "Profile...": startet Profilverwaltung (siehe \abbildung{mockupprofilverwaltung})
\item Unterpunkt "Karten...": startet Kartenverwaltung (siehe \abbildung{mockupkartenverwaltung})
...
\begin{enumerate}
\item Profil-Auswahl (Combobox, Dropdownmenü): legt ein Profil fest
\item Löschen-Button: löscht das aktuell ausgewählte Profil; bei Standardprofilen deaktiviert (\ref{WF:loescheProfil}); zeigt vorher ein Popup an, das den Nutzer davor warnt, dass er viel Berechnungsarbeit zerstört. 
...
\item OK-Button: speichert Änderungen am ausgewählten Profil; setzt es als das aktuelle Profil; schließt den Dialog
\item Abbrechen-Button: verwirft Änderungen am ausgewählten Profil; schließt den Dialog
\end{enumerate}
...
\end{figure}
\begin{enumerate}
\item Karten-Auswahl(Combobox, Dropdownmenü): wählt eine Karte aus.
\item Import-Button: fragt den Benutzer nach einer \gls{osm}-Datei und einem Namen; fügt sie in die Karten-Auswahl ein
\item Aktualisieren-Button: fragt den Benutzer nach einer \gls{osm}-Datei; ersetzt die ausgewählte Karte mit dieser
...
\item Hinzufügen-Button: fügt ein Profil zur Profil-Liste der ausgewählten Karte hinzu\\
...
\end{figure}
\begin{enumerate}
\item Verlauf-Liste: zeigt eine Liste aller \gls{verlauf}-Einträge; der Neueste ganz oben
\item Scrollbalken: erlaubt durch die Verlauf-Liste zu scrollen
\item OK-Button: setzt den aktuellen Start- und Zielpunkt nach dem ausgewählten Verlauf-Eintrag; schließt den Dialog
\item Abbrechen-Button: schließt den Dialog
\end{enumerate}
...
\testfall
    ...
    {Karte ist sichtbar}
    {Ziehen der Karte}
    {Die Kartenposition hat sich entsprechend verändert}
...
Mit einem Rechtsklick öffnet sie das Kontextmenü und wählt diesen Punkt als Startpunkt. Die Koordinaten des Startpunkts werden im in dem dafür vorgesehen Feld angezeigt. Um den Zielpunkt auszuwählen tippt Alice in das Feld Ziel die Koordinaten \texttt{49,01386 8,38106} ein. 
