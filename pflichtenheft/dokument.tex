...
\newglossaryentry{dezimalkoordinaten}{
  ...
  description={Ein Format für Geokoordinaten, nämlich:\\
  	...
  	die jeweils der Form "$[+-] [\text{0-9}]^+.[\text{0-9}]^\ast$" sind}
}
...
\section{Einleitung}
% Betreuer2: Vorberechnung dauert lange. "Experterfeature"
...
\oitem{WK}{karteRendernDeko} Damit der Benutzer sieht dass das Programm nicht hängt  .... %TODO
...
% Felix
% Betreuer2: Profil wählen.
% Betreuer2: Ausfteilen Wusch/Muss.
% Betreuer2: Mehr punke
% Betreuer2: Maker anzeigen.
% Betreuer2: Vorberechnung Überschrift
...
% Betreuer2: Bestimmen, Berechnen, nicht Stätzen
\oitem{F}{zeitSchaetzen}
Schätzen der benötigten Zeit für ein Wegstück unter Berücksichtigung des {profil}{Profils}, der Ampeln, der Abbiegezeiten und des Straßentyps.
\oitem{F}{gpxAusRoute}
...
\oitem{F}{htmlAusBeschreibung}
Erzeugung einer HTML-Datei aus einer \gls{wegbeschreibung}
\end{description}

\subsection{Profilverwaltung}
\begin{description}
\oitem{F}{neuesProfil} Anlegen eines neuen Profils
\oitem{F}{loescheProfil} Löschen eines vorhandenen Profils
\oitem{F}{aendereProfil} Ändern eines vorhandenen Profils
% Betreuer2: Answählen
\end{description}
...
Die Hauptsteuerklasse des Programms. Initialinsiert die Anwendung.
\oitem{PK}{gui} GUI\\
...
\item Löschen-Button: Löscht das aktuell ausgewählte Profil; bei Standardprofilen deaktiviert (\ref{F:loescheProfil})
\item Neu-Button: Erstellt eine Kopie des aktuellen Profils; den Namen gibt der Benutzer in einem weiteren Dialog ein (\ref{F:neuesProfil})
...
\item Abbrechen-Butten: Verwirft Änderungen am ausgewählten Profil und schließt den Dialog
\end{enumerate}
