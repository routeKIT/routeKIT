...
\newglossaryentry{osmkachel}{
  name={OSM-Kachel},
  description={Die öffentlichen gerenderten \gls{OSM}-Kacheln z.B. auf  	http://a.tile.openstreetmap.org/ },
  plural={OSM-Kacheln}
}
...
Die zuletzt benutzte Karte wird über mehrere Programmstarte behalten.
\oitem{WF}{letztesProfil}
Das zuletzt benutzte \gls{profil} wird über mehrere Programmstarte behalten.
\end{description}
...
\oitem{PK}{gui} GUI\\
Enthält alle Komponenten des GUI außer \ref{PK:mapView}. Realisiert u.a \ref{MK:startZielWaehlen}.
...
\begin{quote}
  ...
  Wenn Sie sicher sind, dass Sie diese Operationen durchführen wollen, geben Sie ein: ``Ja, ich weiß, was ich tue.''
\end{quote}
Dabei wird der Dialog bewusst sehr aufdringlich gestaltet, um zu vermeiden, dass der Benutzer aus Versehen die Operationen durchführt.
% TODO labeln
% Fabian
...
\oitem{TF}{exitstartHistory}\testfall
...
\oitem{TF}{exitstartBerechnung}\testfall
    {Karte ist sichtbar, aktuelles Profil ist nicht vorberechnet (für die aktuelle Karte)}
    {Der Benutzer berechnet eine Profil:Karten-Kombination, wartet, beendet das Programm, startet das Programm}
    {Das zuvor berechnete Profil ist immer noch berechnet}
\oitem{TF}{exportGPX}\testfall
...
\oitem{TF}{eigeneKacheln}\testfall
...
\oitem{TF}{loescheProfil}\testfall
    ...
    {Ein Profil, das nicht das Standartprofil ist, ist vorhanden}
    {Das Profil wird gelöscht}
    {Das Profil ist nicht mehr vorhanden}
\oitem{TF}{aendereProfil}\testfall
    ...
    {Das Profil die neuen Daten}
\oitem{TF}{profilPersistenz}\testfall
    {Ein Profil ist vorhanden}
...
\oitem{TF}{loescheKarte}\testfall
    {Eine andere Karte als die Standartkarte ist vorhanden}
    {Die Karte wird gelöscht}
    {Die Karte ist nicht mehr vorhanden}
\oitem{TF}{kartenPersistenz}\testfall
    {-}
...
\oitem{TF}{abbiegebeschraenkung}\testfall
...
\oitem{TF}{einschraenkungNeg}\testfall
    ...
    {Ein Weg, der nicht diese Straße enthält, wird gefunden}
\oitem{TF}{einschraenkungPos}\testfall
